...
const CLASS_NAME_RECT = 'LineChartSelectionX__rect';
const CLASS_NAME_LINE_LEFT = 'LineChartSelectionX__line_left';
const CLASS_NAME_LINE_RIGHT = 'LineChartSelectionX__line_right';
...
export const LineChartSelectionX = React.memo(({ anchorEl, height, scale, data, onSelect }: LineChartSelectionXProps) => {
  ...
  const drawSelection = React.useCallback((x0: number, x1: number) => {
    ...
    d3.select(ref.current)
      .select(`.${ CLASS_NAME_LINE_LEFT }`)
      .attr('x1', x0)
      .attr('x2', x0);

    d3.select(ref.current)
      .select(`.${ CLASS_NAME_LINE_RIGHT }`)
      .attr('x1', x1)
      .attr('x2', x1);

    d3.select(ref.current)
      .select(`.${ CLASS_NAME_RECT }`)
  ...
  return (
    <g className="ChartSelectionX" ref={ ref } opacity={ 0 }>
      <rect className={ CLASS_NAME_RECT } width={ 0 } height={ height } fill="rgba(66, 153, 225, 0.1)"/>
      <line className={ CLASS_NAME_LINE_LEFT } x1={ 0 } x2={ 0 } y1={ 0 } y2={ height } stroke={ borderColor }/>
      <line className={ CLASS_NAME_LINE_RIGHT } x1={ 0 } x2={ 0 } y1={ 0 } y2={ height } stroke={ borderColor }/>
    </g>
  );
});
