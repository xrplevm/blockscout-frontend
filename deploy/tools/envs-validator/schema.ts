...
const marketplaceAppSchema: yup.ObjectSchema<MarketplaceAppOverview> = yup.object({
  ...
  github: yup.lazy((value) => (Array.isArray(value) ? yup.array().of(yup.string().required().test(urlTest)) : yup.string().test(urlTest))),
  discord: yup.string().test(urlTest),
  internalWallet: yup.boolean(),
  priority: yup.number(),
});

const issueSeverityDistributionSchema: yup.ObjectSchema<MarketplaceAppSecurityReport["overallInfo"]["issueSeverityDistribution"]> =
  yup.object({
...
const solidityscanReportSchema: yup.ObjectSchema<MarketplaceAppSecurityReport["contractsData"][number]["solidityScanReport"]> = yup.object({
...
const contractDataSchema: yup.ObjectSchema<MarketplaceAppSecurityReport["contractsData"][number]> = yup.object({
...
const securityReportSchema: yup.ObjectSchema<MarketplaceAppSecurityReportRaw> = yup.object({
...
const marketplaceSchema = yup.object().shape({
  ...
  NEXT_PUBLIC_MARKETPLACE_CONFIG_URL: yup
    ...
    .when("NEXT_PUBLIC_MARKETPLACE_ENABLED", {
      ...
      otherwise: (schema) =>
        schema.max(-1, "NEXT_PUBLIC_MARKETPLACE_CONFIG_URL cannot not be used without NEXT_PUBLIC_MARKETPLACE_ENABLED"),
    }),
  NEXT_PUBLIC_MARKETPLACE_CATEGORIES_URL: yup
    ...
    .when("NEXT_PUBLIC_MARKETPLACE_ENABLED", {
      ...
      // eslint-disable-next-line max-len
      otherwise: (schema) =>
        schema.max(-1, "NEXT_PUBLIC_MARKETPLACE_CATEGORIES_URL cannot not be used without NEXT_PUBLIC_MARKETPLACE_ENABLED"),
    }),
  NEXT_PUBLIC_MARKETPLACE_SUBMIT_FORM: yup.string().when("NEXT_PUBLIC_MARKETPLACE_ENABLED", {
    is: true,
  ...
  NEXT_PUBLIC_MARKETPLACE_SUGGEST_IDEAS_FORM: yup.string().when("NEXT_PUBLIC_MARKETPLACE_ENABLED", {
    is: true,
    then: (schema) => schema.test(urlTest),
    // eslint-disable-next-line max-len
    otherwise: (schema) =>
      schema.max(-1, "NEXT_PUBLIC_MARKETPLACE_SUGGEST_IDEAS_FORM cannot not be used without NEXT_PUBLIC_MARKETPLACE_ENABLED"),
  }),
  NEXT_PUBLIC_MARKETPLACE_SECURITY_REPORTS_URL: yup
    ...
    .when("NEXT_PUBLIC_MARKETPLACE_ENABLED", {
      ...
      // eslint-disable-next-line max-len
      otherwise: (schema) =>
        schema.max(-1, "NEXT_PUBLIC_MARKETPLACE_SECURITY_REPORTS_URL cannot not be used without NEXT_PUBLIC_MARKETPLACE_ENABLED"),
    }),
  NEXT_PUBLIC_MARKETPLACE_FEATURED_APP: yup.string().when("NEXT_PUBLIC_MARKETPLACE_ENABLED", {
    is: true,
    then: (schema) => schema,
    // eslint-disable-next-line max-len
    otherwise: (schema) =>
      schema.max(-1, "NEXT_PUBLIC_MARKETPLACE_FEATURED_APP cannot not be used without NEXT_PUBLIC_MARKETPLACE_ENABLED"),
  }),
  NEXT_PUBLIC_MARKETPLACE_BANNER_CONTENT_URL: yup.string().when("NEXT_PUBLIC_MARKETPLACE_ENABLED", {
    is: true,
    then: (schema) => schema.test(urlTest),
    // eslint-disable-next-line max-len
    otherwise: (schema) =>
      schema.max(-1, "NEXT_PUBLIC_MARKETPLACE_BANNER_CONTENT_URL cannot not be used without NEXT_PUBLIC_MARKETPLACE_ENABLED"),
  }),
  NEXT_PUBLIC_MARKETPLACE_BANNER_LINK_URL: yup.string().when("NEXT_PUBLIC_MARKETPLACE_ENABLED", {
    is: true,
    then: (schema) => schema.test(urlTest),
    // eslint-disable-next-line max-len
    otherwise: (schema) =>
      schema.max(-1, "NEXT_PUBLIC_MARKETPLACE_BANNER_LINK_URL cannot not be used without NEXT_PUBLIC_MARKETPLACE_ENABLED"),
  }),
  NEXT_PUBLIC_MARKETPLACE_RATING_AIRTABLE_API_KEY: yup.string().when("NEXT_PUBLIC_MARKETPLACE_ENABLED", {
    is: true,
    then: (schema) => schema,
    // eslint-disable-next-line max-len
    otherwise: (schema) =>
      schema.max(-1, "NEXT_PUBLIC_MARKETPLACE_RATING_AIRTABLE_API_KEY cannot not be used without NEXT_PUBLIC_MARKETPLACE_ENABLED"),
  }),
  NEXT_PUBLIC_MARKETPLACE_RATING_AIRTABLE_BASE_ID: yup.string().when("NEXT_PUBLIC_MARKETPLACE_ENABLED", {
    is: true,
    then: (schema) => schema,
    // eslint-disable-next-line max-len
    otherwise: (schema) =>
      schema.max(-1, "NEXT_PUBLIC_MARKETPLACE_RATING_AIRTABLE_BASE_ID cannot not be used without NEXT_PUBLIC_MARKETPLACE_ENABLED"),
  }),
  NEXT_PUBLIC_MARKETPLACE_GRAPH_LINKS_URL: yup.string().when("NEXT_PUBLIC_MARKETPLACE_ENABLED", {
    is: true,
    then: (schema) => schema,
    // eslint-disable-next-line max-len
    otherwise: (schema) =>
      schema.max(-1, "NEXT_PUBLIC_MARKETPLACE_GRAPH_LINKS_URL cannot not be used without NEXT_PUBLIC_MARKETPLACE_ENABLED"),
  }),
});

const beaconChainSchema = yup.object().shape({
  NEXT_PUBLIC_HAS_BEACON_CHAIN: yup.boolean(),
  NEXT_PUBLIC_BEACON_CHAIN_CURRENCY_SYMBOL: yup.string().when("NEXT_PUBLIC_HAS_BEACON_CHAIN", {
    is: (value: boolean) => value,
    then: (schema) => schema.min(1).optional(),
    otherwise: (schema) =>
      schema.max(-1, 'NEXT_PUBLIC_BEACON_CHAIN_CURRENCY_SYMBOL cannot not be used if NEXT_PUBLIC_HAS_BEACON_CHAIN is not set to "true"'),
  }),
});
...
const rollupSchema = yup.object().shape({
  ...
  NEXT_PUBLIC_ROLLUP_L2_WITHDRAWAL_URL: yup.string().when("NEXT_PUBLIC_ROLLUP_TYPE", {
    is: (value: string) => value === "optimistic",
    then: (schema) => schema.test(urlTest).required(),
    otherwise: (schema) =>
      schema.max(-1, "NEXT_PUBLIC_ROLLUP_L2_WITHDRAWAL_URL can be used only if NEXT_PUBLIC_ROLLUP_TYPE is set to 'optimistic' "),
  }),
  NEXT_PUBLIC_ROLLUP_HOMEPAGE_SHOW_LATEST_BLOCKS: yup.boolean().when("NEXT_PUBLIC_ROLLUP_TYPE", {
    is: (value: string) => value,
    then: (schema) => schema,
    otherwise: (schema) =>
      schema.test(
...
const celoSchema = yup.object().shape({
  NEXT_PUBLIC_CELO_ENABLED: yup.boolean(),
  NEXT_PUBLIC_CELO_L2_UPGRADE_BLOCK: yup.string().when("NEXT_PUBLIC_CELO_ENABLED", {
    is: (value: boolean) => value,
    then: (schema) => schema.min(0).optional(),
    otherwise: (schema) =>
      schema.max(-1, 'NEXT_PUBLIC_CELO_L2_UPGRADE_BLOCK cannot not be used if NEXT_PUBLIC_CELO_ENABLED is not set to "true"'),
  }),
});
...
const sentrySchema = yup.object().shape({
  ...
  NEXT_PUBLIC_SENTRY_ENABLE_TRACING: yup.boolean().when("NEXT_PUBLIC_SENTRY_DSN", {
    is: (value: string) => Boolean(value),
    then: (schema) => schema,
  }),
...
const accountSchema = yup.object().shape({
  ...
  NEXT_PUBLIC_AUTH0_CLIENT_ID: yup.string().when("NEXT_PUBLIC_IS_ACCOUNT_SUPPORTED", {
    is: (value: boolean) => value,
    then: (schema) => schema,
    otherwise: (schema) =>
      schema.max(-1, 'NEXT_PUBLIC_AUTH0_CLIENT_ID cannot not be used if NEXT_PUBLIC_IS_ACCOUNT_SUPPORTED is not set to "true"'),
  }),
  NEXT_PUBLIC_AUTH_URL: yup.string().when("NEXT_PUBLIC_IS_ACCOUNT_SUPPORTED", {
    is: (value: boolean) => value,
    then: (schema) => schema.test(urlTest),
    otherwise: (schema) =>
      schema.max(-1, 'NEXT_PUBLIC_AUTH_URL cannot not be used if NEXT_PUBLIC_IS_ACCOUNT_SUPPORTED is not set to "true"'),
  }),
  NEXT_PUBLIC_LOGOUT_URL: yup.string().when("NEXT_PUBLIC_IS_ACCOUNT_SUPPORTED", {
    is: (value: boolean) => value,
    then: (schema) => schema.test(urlTest),
    otherwise: (schema) =>
      schema.max(-1, 'NEXT_PUBLIC_LOGOUT_URL cannot not be used if NEXT_PUBLIC_IS_ACCOUNT_SUPPORTED is not set to "true"'),
  }),
});

const featuredNetworkSchema: yup.ObjectSchema<FeaturedNetwork> = yup.object().shape({
  title: yup.string().required(),
  url: yup.string().test(urlTest).required(),
...
const heroBannerSchema: yup.ObjectSchema<HeroBannerConfig> = yup
  .object()
  .transform(replaceQuotes)
  .json()
  .shape({
...
const bridgedTokensSchema = yup.object().shape({
  NEXT_PUBLIC_BRIDGED_TOKENS_CHAINS: yup.array().transform(replaceQuotes).json().of(bridgedTokenChainSchema),
  NEXT_PUBLIC_BRIDGED_TOKENS_BRIDGES: yup
    ...
    .when("NEXT_PUBLIC_BRIDGED_TOKENS_CHAINS", {
      ...
      otherwise: (schema) =>
        schema.max(-1, "NEXT_PUBLIC_BRIDGED_TOKENS_BRIDGES cannot not be used without NEXT_PUBLIC_BRIDGED_TOKENS_CHAINS"),
    }),
});
...
const schema = yup
  ...
  .shape({
    ...
    // 4. UI configuration
    //   a. homepage
    NEXT_PUBLIC_HOMEPAGE_CHARTS: yup.array().transform(replaceQuotes).json().of(yup.string<ChainIndicatorId>().oneOf(CHAIN_INDICATOR_IDS)),
    ...
    NEXT_PUBLIC_HOMEPAGE_HERO_BANNER_CONFIG: yup.mixed().test(
      "shape",
      (ctx) => {
        try {
          ...
          const message =
            typeof error === "object" && error !== null && "errors" in error && Array.isArray(error.errors) ? error.errors.join(", ") : "";
    ...
    NEXT_PUBLIC_OTHER_LINKS: yup.array().transform(replaceQuotes).json().of(navItemExternalSchema),
    NEXT_PUBLIC_NAVIGATION_HIDDEN_LINKS: yup
      .array()
      .transform(replaceQuotes)
      .json()
      .of(yup.string<NavigationLinkId>().oneOf(NAVIGATION_LINK_IDS)),
    NEXT_PUBLIC_NAVIGATION_HIGHLIGHTED_ROUTES: yup.array().transform(replaceQuotes).json().of(yup.string()),
    ...
    NEXT_PUBLIC_VIEWS_ADDRESS_FORMAT: yup.array().transform(replaceQuotes).json().of(yup.string<AddressFormat>().oneOf(ADDRESS_FORMATS)),
    NEXT_PUBLIC_VIEWS_ADDRESS_BECH_32_PREFIX: yup.string().when("NEXT_PUBLIC_VIEWS_ADDRESS_FORMAT", {
      is: (value: Array<AddressFormat> | undefined) => value && value.includes("bech32"),
      then: (schema) => schema.required().min(1).max(83),
      otherwise: (schema) =>
        schema.max(-1, 'NEXT_PUBLIC_VIEWS_ADDRESS_BECH_32_PREFIX is required if NEXT_PUBLIC_VIEWS_ADDRESS_FORMAT contains "bech32"'),
    }),
    ...
    NEXT_PUBLIC_VIEWS_TX_HIDDEN_FIELDS: yup.array().transform(replaceQuotes).json().of(yup.string<TxFieldsId>().oneOf(TX_FIELDS_IDS)),
    NEXT_PUBLIC_VIEWS_TX_ADDITIONAL_FIELDS: yup
      .array()
      .transform(replaceQuotes)
      .json()
      .of(yup.string<TxAdditionalFieldsId>().oneOf(TX_ADDITIONAL_FIELDS_IDS)),
    NEXT_PUBLIC_VIEWS_NFT_MARKETPLACES: yup.array().transform(replaceQuotes).json().of(nftMarketplaceSchema),

    //     e. misc
    NEXT_PUBLIC_NETWORK_EXPLORERS: yup.array().transform(replaceQuotes).json().of(networkExplorerSchema),
    NEXT_PUBLIC_CONTRACT_CODE_IDES: yup.array().transform(replaceQuotes).json().of(contractCodeIdeSchema),
    ...
    NEXT_PUBLIC_FONT_FAMILY_BODY: yup.mixed().test("shape", "Invalid schema were provided for NEXT_PUBLIC_FONT_FAMILY_BODY", (data) => {
      const isUndefined = data === undefined;
      return isUndefined || fontFamilySchema.isValidSync(data);
    }),
    ...
    NEXT_PUBLIC_WEB3_WALLETS: yup
      ...
      .test(
        ...
        (data) => {
          ...
          const isArrayOfWalletsSchema = yup.array().transform(replaceQuotes).json().of(yup.string<WalletType>().oneOf(SUPPORTED_WALLETS));
    ...
    NEXT_PUBLIC_MULTICHAIN_BALANCE_PROVIDER_CONFIG: yup.array().transform(replaceQuotes).json().of(multichainProviderConfigSchema),
    ...
    NEXT_PUBLIC_DEFI_DROPDOWN_ITEMS: yup.array().transform(replaceQuotes).json().of(deFiDropdownItemSchema),
    NEXT_PUBLIC_FAULT_PROOF_ENABLED: yup.boolean().when("NEXT_PUBLIC_ROLLUP_TYPE", {
      is: "optimistic",
      then: (schema) => schema,
      otherwise: (schema) =>
        schema.test(
    ...
    NEXT_PUBLIC_HAS_MUD_FRAMEWORK: yup.boolean().when("NEXT_PUBLIC_ROLLUP_TYPE", {
      is: "optimistic",
      then: (schema) => schema,
      otherwise: (schema) =>
        schema.test(
