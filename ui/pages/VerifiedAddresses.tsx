...
const VerifiedAddresses = () => {
  ...
  if (selectedAddress) {
    const addressInfo = addressesQuery.data?.verifiedAddresses.find(
      ({ contractAddress }) => contractAddress.toLowerCase() === selectedAddress.toLowerCase(),
    );
      ...
      <PeersystPageWrapper>
        ...
        <TokenInfoForm
          ...
          application={ applicationsQuery.data?.submissions.find(
            ({ tokenAddress }) => tokenAddress.toLowerCase() === selectedAddress.toLowerCase(),
          ) }
      ...
      <Show below="lg" key="content-mobile" ssr={ false }>
        { addressesQuery.data.verifiedAddresses.map((item, index) => (
          <VerifiedAddressesListItem
            ...
            item={ item }
            application={ applicationsQuery.data?.submissions?.find(
              ({ tokenAddress }) => tokenAddress.toLowerCase() === item.contractAddress.toLowerCase(),
            ) }
            onAdd={ handleItemAdd }
  ...
  return (
    <PeersystPageWrapper>
      ...
      <AccountPageDescription allowCut={ false }>
        <span>
          Verify ownership of a smart contract address to easily update information in Blockscout. You will sign a single message to verify
          contract ownership. Once verified, you can update token information, address name tags, and address labels from the Blockscout
          console without needing to sign additional messages.
        </span>
        ...
        <OrderedList ml={ 6 }>
          ...
          <ListItem>
            ...
            <Link href="https://docs.blockscout.com/for-users/verifying-a-smart-contract" target="_blank">
              this tool
            </Link>
            <span>).</span>
          </ListItem>
        </OrderedList>
        <chakra.div mt={ 5 }>Once these steps are complete, click the Add address button below to get started.</chakra.div>
